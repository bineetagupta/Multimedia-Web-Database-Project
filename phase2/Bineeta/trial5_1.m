...
clear all;
r=4;
fileName='C:\Users\Binee\Desktop\M.csv';
delimiter=',';
...
columns_of_videoVector1=size(videoVector1,2);

new_matrix_video1=zeros(9500,columns_of_videoVector1);
...
global_count=1;
size_of_list2=size(list_of_video2,1);
new_matrix_video2=zeros(9500,11);
previous_count=0;
start_from=0;
end_at=0;
...
final_matrix=zeros(72,72);maxx=0;minn=0;
mag1_1=0; mag1_2=0;mag2_1=0;mag2_2=0;
mag3_1=0;mag3_2=0;mag4_1=0;mag4_2=0;
c= (r*r);

%final matrix construction using intersection similarity
for i_new=0:nframes1
    for j_new=0:nframes2
        dot=0;
        maxx=0;minn=0;
        for c_new=1:c
            mag1_1=new_matrix_video1_HOMV(c*(i_new)+c_new,2);
            mag1_2=new_matrix_video2_HOMV(c*(j_new)+c_new,2);
            mag2_1=new_matrix_video1_HOMV(c*(i_new)+c_new,3);
            mag2_2=new_matrix_video2_HOMV(c*(j_new)+c_new,3);
            mag3_1=new_matrix_video1_HOMV(c*(i_new)+c_new,4);
            mag3_2=new_matrix_video2_HOMV(c*(j_new)+c_new,4);
            mag4_1=new_matrix_video1_HOMV(c*(i_new)+c_new,5);
            mag4_2=new_matrix_video2_HOMV(c*(j_new)+c_new,5);
         ...
         if (maxx==0)
            ...
            final_matrix(i_new+1,j_new+1)=1-dot;
     end
end

dynamicAllign(final_matrix)