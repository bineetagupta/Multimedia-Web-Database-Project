function dist=dynamicMultiple(matrix,kk)
...
for i=2:nFrames1+1
    for j=2:nFrames2+1
        matrixD(i,j)= min([matrixD(i-1,j-1)+matrix(i-1,j-1)*w2,matrixD(i-1,j)+matrix(i-1,j-1)*w1,matrixD(i,j-1)+matrix(i-1,j-1)*w1]);
    end
end
dist=matrixD(max(2,nFrames1+1-kk):nFrames1+1,nFrames2+1);
