...
{
	string record;
	fin >> record;//discard the first header line
	while(fin>>record)
	{
...
void parser::tokenize_record(const string & record)
{
	try
	{
		...
		if(std::getline(ss, token, ','))
		{
			...
			//string tok;
			curr_frame->cells[c-1].hist.push_back(atoi(token.c_str()));
			for(int j=1;j<n;j++)
			{
...
double parser::frame_dist_intersect(t_frame& f1, t_frame& f2)
{
	...
	for(int i=0;i<total_cells;i++)
	{
		cell_dist=cell_dist_intersect(f1.cells[i], f2.cells[i],f1_pix_cnt,f2_pix_cnt);
		if(cell_dist>max_dist)
			max_dist=cell_dist;
		dist+=cell_dist;
	}
...
double parser::get_score(int v1, int v2)
{
	...
	if(vit1!=videos.end() && vit2!=videos.end())
	{
		...
		//print out the value of the min dist path which is: dist_matrix[m-1][n-1]
		//cout<<"\nScore for video "<<v1<<" and video "<<v2<<" : "<<setprecision(8)<<dist_matrix[m-1][n-1]<<endl;
		cout<<dist_matrix[m-1][n-1];
	}
}

int main(int argc, char* argv[])
{
	//input_filename - full_path, video_num1, video_num2, r, n 
	int r,n;
	
	fin.open(argv[1]);
	//fin >> "Writing this to a file.\n";
	...
	if(argc>=4 && fin.is_open())
	{
	    fin >>r>>n;// see if this would work
	    parser p=parser(r,n);
	...
	{
		p.cal_subseq(vi, i, a, b, top_k_map);
	}
	
	int i=0;
	
	for(t_top_k_map::iterator it=top_k_map.begin(); it!=top_k_map.end() && i<k; it++, i++)
	{
