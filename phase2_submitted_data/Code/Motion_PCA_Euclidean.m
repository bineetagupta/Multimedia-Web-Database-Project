function final_distance = Motion_PCA_Euclidean(fileName,file1,file2)
...
video2_cell_count_list=zeros(global_count,1);index_video2_cell_count_list=1;ii=1;sum=0;
...
for i_new=0:(nframes1-1)
    for j_new=0:(nframes2-1)
        dot=0;
        for c_new=1:r
          dot=dot+pdist2(new_vector_1_norm(r*(i_new)+c_new,:),new_vector_2_norm(r*(j_new)+c_new,:));
